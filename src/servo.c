...
ISR(TIMER1_OVF_vect)
{
	...
	else	// regular start of a new pulse
	{
		if(servo_value[current_servo] != SERVO_NO_PULSE) // regular pulse value
		{
			...
			// If we're going to control the 12th and 13th servo from the slave,
			//we need reverse servo logic. (But only for those

			// We only reverse the direction of the servo's for non HP servos!
			if (current_servo > 5){
